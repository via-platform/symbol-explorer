...
const SymbolExplorerSymbol = require('./symbol-explorer-symbol');

module.exports = class SymbolExplorerCategory {
    constructor({path, isRoot, expansionState = {}, explorer}){
    ...
    watch(){
        this.watchSubscription = new CompositeDisposable(
            via.markets.onDidCreateMarket(market => {
                if(market.categories.filter(path => path.indexOf(this.path) === 0).length){
                    this.reload();
                }
            }),
            via.markets.onDidDestroyMarket(market => {
                if(market.categories.filter(path => path.indexOf(this.path) === 0).length){
                    this.reload();
                }
            })
    ...
    getEntries(){
        let markets = via.markets.all();
        let paths = markets.map(market => market.categories).reduce((a, b) => a.concat(b), []);
        ...
        for(let path of descendents){
            ...
            if(this.entries.has(path)){
                ...
                categoriesToDisplay.push(new SymbolExplorerCategory({path, expansionState, explorer: this.explorer}));
            }
        }

        for(const market of markets){
            if(this.isIgnored(market.symbol)){
                continue;
            }

            if(market.categories.includes(this.path)){
                if(this.entries.has(market.symbol)){
                    symbolsToDisplay.push(market.symbol);
                }else{
                    symbolsToDisplay.push(new SymbolExplorerSymbol({
                        market,
                        path: this.path + '/' + market.symbol,
    ...
    sortEntries(combinedEntries){
        if(via.config.get('symbol-explorer.sortCategoriesBeforeSymbols')){
