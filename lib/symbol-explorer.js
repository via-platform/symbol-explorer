...
module.exports = class SymbolExplorer {
    constructor(state = {}) {
        ...
        this.header.classList.add('symbol-explorer-header');
    ...
    handleEvents(){
        ...
        this.disposables.add(via.symbols.onDidAddSymbol(this.didAddSymbol.bind(this)));
    ...
    didAddSymbol(symbol){
        const symbols = via.symbols.getSymbols();
        this.header.textContent = symbols.length ? `${symbols.length} Symbols` : `No Symbols`;
