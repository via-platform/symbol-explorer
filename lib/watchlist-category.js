...
module.exports = class WatchlistCategory {
    constructor({path, isRoot, expansionState}){
        ...
        this.expansionState.isExpanded = this.expansionState.isExpanded || false;
        this.expansionState.entries = this.expansionState.entries || new Map();
        this.status = null;
    ...
    directDescendents(paths){
        paths = paths
            ...
            .map(path => this.path + '/' + path.slice(this.id.length + 1).split('/').shift());
    ...
    getEntries(){
        ...
        for(let path of descendents){
            ...
            if(this.entries.has(path)){
                ...
                categoriesToDisplay.push(new WatchlistCategory({path, expansionState}));
            }
        }
        ...
        for(let symbol of symbols){
            ...
            if(symbol.categories.includes(this.path)){
                if(this.entries.has(symbol.id)){
                    ...
                    symbolsToDisplay.push(new WatchlistSymbol({symbol, path: this.path + '/' + symbol.id}));
                }
            }
        }
    ...
    reload(){
        ...
        if(newEntries.length > 0){
            newEntries.forEach(entry => this.entries.set(entry.name, entry));
