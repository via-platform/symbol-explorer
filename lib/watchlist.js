...
module.exports = class Watchlist {
    ...
    attachFavorites(){
        try {
            ...
            this.list.appendChild(favoritesView.element);
        }catch(e){
            console.log(e);
        }
    }
    ...
    watchlistSymbolViewEntryClicked(e){
        ...
        if(e.target.classList.contains('favorite-icon')){
            if(this.favorites.includes(symbol.identifier)){
                this.favorites.splice(this.favorites.indexOf(symbol.identifier), 1);
                this.emitter.emit('did-remove-favorite', symbol);
            }else{
                this.favorites.push(symbol.identifier);
